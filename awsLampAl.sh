...
# 3. Launch EC2 instance using Amazon Linux 2023
echo "Finding the latest Amazon Linux 2023 AMI in the current region..."
AMI_ID=$(aws ec2 describe-images \
    --owners amazon \
    --filters 'Name=name,Values=amzn2023-ami-minimal-*' \
...
count=0
while ! ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no -i ~/.ssh/key_WebServerAuto ec2-user@$ELASTIC_IP 'exit' 2>/dev/null
do
    count=$((count+1))
...
if [ "'"$INSTALL_LAMP"'" = true ]; then
    ...
    # Adjust DirectoryIndex in the httpd config if needed (path may vary)
...
if [ "'"$INSTALL_VSCODE"'" = true ]; then
    ...
    # Add your VS Code Server installation commands here (if required)
fi
...
if [ "'"$INSTALL_DB"'" = true ]; then
    ...
    # Link or configure Adminer if needed (path may vary)
...
if [ "'"$INSTALL_DB"'" = true ]; then    
    printf "\nOpen an internet browser (e.g. Chrome) and go to \e[3;4;33mhttp://$(curl -s ifconfig.me)/adminer/?username=admin\e[0m - You should see the Adminer Login page. Username is admin and password is password. Leave Database empty.\n"
    printf "\nOpen an internet browser (e.g. Chrome) and go to \e[3;4;33mhttp://$(curl -s ifconfig.me)/phpmyadmin\e[0m - You should see the phpMyAdmin login page. (admin/password)\n"
fi
if [ "'"$INSTALL_WORDPRESS"'" = true ]; then
    printf "\nOpen an internet browser (e.g. Chrome) and go to \e[3;4;33mhttp://$(curl -s ifconfig.me)\e[0m - You should see the WordPress page.\n"
    printf "\nOpen an internet browser (e.g. Chrome) and go to \e[3;4;33mhttp://$(curl -s ifconfig.me)/wp-admin\e[0m - You should see the WordPress Dashboard (admin/password)\n"
fi
if [ "'"$INSTALL_MATOMO"'" = true ]; then
    printf "\nOpen an internet browser (e.g. Chrome) and go to \e[3;4;33mhttp://$(curl -s ifconfig.me)/matomo\e[0m - You should see the Matomo Install page.\n"
fi
